        ...
        $loop->addPeriodicTimer(2, function () {
            ...
            $this->fetchCoasters()->then(function ($stats) use ($dateTime) {
                ...
                foreach ($stats as $coasterModel) {
                    ...
                    CLI::write(lang('Messages.coaster.header', ['name' => $coasterModel->uuid]));
                    CLI::write(lang('Messages.coaster.opening_hours', ['from' => $coasterModel->hours_from, 'to' => $coasterModel->hours_to]));
                    CLI::write(lang('Messages.coaster.wagons', ['wagons' => $coasterModel->getWagons()->count(), 'expected' => $coasterModel->getExpectedNumberOfWagons() ]));
                    CLI::write(lang('Messages.coaster.staff', ['staff' => $coasterModel->number_of_staff, 'required_staff' => $coasterModel->getRequiredStaff()]));
